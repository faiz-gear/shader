#iChannel0 "https://i.imgur.com/3gnoIwS.jpg"
#iChannel1 "https://i.imgur.com/WN7QOVa.jpg"
...
vec4 dissolve(vec2 uv){
    ...
    // 亮边的长度和颜色。
    float edgeWidth=.1;
    vec3 edgeColor=vec3(0.051, 0.8353, 0.7059);
    float edge=1.-smoothstep(0.,edgeWidth,noise-progress);
    // col.rgb=vec3(edge);
    // col.rgb=mix(col.rgb,edgeColor,edge);
    col.rgb=mix(col.rgb,edgeColor,edge*step(.0001,progress)); // 消除鼠标到最左侧的亮边
